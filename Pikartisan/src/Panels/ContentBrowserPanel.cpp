...
namespace Pika {
	...
	{
		if (m_IsShowContentBrowser) {
			ImGui::Begin("Content Browser", &m_IsShowContentBrowser);
			...
			for (auto& DirectoryEntry : std::filesystem::directory_iterator(m_CurrentDirectory)) {
				...
				if (ImGui::BeginDragDropSource()) {
					...
					ImGui::SetDragDropPayload("CONTENT_BROWSER_ITEM", ItemPath.c_str(), ItemPath.size() + 1); // +1包含终止符'\0'
