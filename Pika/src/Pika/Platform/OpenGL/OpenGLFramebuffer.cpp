...
namespace Pika {
	...
	{
		PK_PROFILE_FUNCTION();
		bind(); // glReadPixels是读取当前绑定的Framebuffer数据，需要绑定
	...
	void OpenGLFramebuffer::setColorAttachment(uint32_t vIndex, const Ref<Texture>& vTexture)
	{
		...
		glBindFramebuffer(GL_FRAMEBUFFER, m_RendererID);
		bool IsMultisample = m_Specification.m_Samples > 1;
		glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0 + vIndex, GL_TEXTURE_2D, vTexture->getRendererID(), 0);
		uint32_t LastColorAttachment = m_ColorAttachments[vIndex];
		if (glCheckFramebufferStatus(GL_FRAMEBUFFER) == GL_FRAMEBUFFER_COMPLETE) {
			m_ColorAttachments[vIndex] = vTexture->getRendererID();
		}
		else {
			glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0 + vIndex, GL_TEXTURE_2D, m_ColorAttachments[vIndex], 0);
			checkFramebufferStatus();
		}
	}
